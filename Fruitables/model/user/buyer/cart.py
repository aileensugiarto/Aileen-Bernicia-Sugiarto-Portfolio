...
def model_add_cart():
  if request.method == "POST":
    ...
    cur.close()
    return redirect(url_for('cart', id_user=id_user))
  return render_template('user/buyer/index.html')

# CART
def model_cart(id_user):
  cur = mysql.connection.cursor()
  ...
  return render_template('user/buyer/cart.html', data_cart=data, total_price=total)
...
def model_delete_cart(id_cart):
  ...
  return redirect(url_for('cart', id_user=session['id_user']))
